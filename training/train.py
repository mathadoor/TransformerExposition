...
example_sentences = ["A man is walking in the street.", "The dog is running in the park.",
                     ...
                     "A little girl climbing into a wooden playhouse.", "Boys dancing on poles in the middle of the night.",
...
if mode != "train":
  ...
  dummy_src = torch.zeros((max_len, 1)).int().to(device)
  dummy_trg = torch.zeros((max_len, 1)).int().to(device)


  torch.onnx.export(model, (dummy_src, dummy_trg), "model.onnx", input_names=["src", "trg"], operator_export_type=torch.onnx.OperatorExportTypes.ONNX,
                    training=torch.onnx.TrainingMode.EVAL, export_params=True)
